...
namespace SensorAPI
{
    namespace Data
    {
        ...
        public class TimedData
        {
            ...
            public static implicit operator string(TimedData data) => data.value.ToString();
        ...
        public class DataList
        {
            ...
            //public TimedData Latest { get { return data[data.Count - 1]; } }
            /** @brief The max number of entries in the list.
             *  When exceeded, the earliest value added is removed
             */
            public int limit;
            public TimedData Latest()
            {
                return data[Count - 1];
            }
            public DateTime LastUpdate()
            {
                return data[Count - 1].time;
            }
            ...
            public static implicit operator string(DataList x) => x.Latest().value.ToString();
            ...
            public double WeightedAverage(double weight)
            {
                ...
                {
                    average += data[i].value * (((i + 1) * weight) - weight + 1);
                    added += ((i + 1) * weight) - weight + 1;
                }
                average /= added;
                return average;
            }

            public double MovingAverage(int range)
            {
                return data.Skip(Count - range - 1).Take(range).Average(x => x.value);
            }
        }
    } // Namespace Data
} // Namespace SensorAPI
